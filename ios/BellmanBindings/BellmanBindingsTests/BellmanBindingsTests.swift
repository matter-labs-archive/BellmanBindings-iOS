...
class BellmanBindingsTests: XCTestCase {
    ...
    func testVerification() {
        ...
        do {
            let path = try downloader.loadFileAsync(url: URL(string: "http://d.zaix.ru/bswT.txt")!)
            ...
            let result = try verifier.verifyWithPrecompiledProof(filename: path,
                                                                 inputs: inputsBytes,
                                                                 engine: Engine.Bn256,
                                                                 proofVec: proofVec)
            print(result)
            ...
            if error is CustomError {
                print((error as! CustomError).description)
            }
            XCTFail()
        }
    }
